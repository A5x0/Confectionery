# Confectionery
Confectionery is the best build configuration system for extensively configurable applications. It is extremely lightweight and fast. It is just one C file so you could, in theory, run gcc main.c and get confectionery, but there are benefits that come with compiling it with the Makefile method such as a test suite (one test is known to fail), installation with make install, and being able to customize the Makefile if you want to.
